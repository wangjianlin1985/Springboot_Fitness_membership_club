...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    onCheck: options.onCheck,                           // 选中事件
                    onUncheck: options.onUncheck,                       // 取消选中事件
                    onCheckAll: options.onCheckAll,                     // 全选事件
                    onUncheckAll: options.onUncheckAll,                 // 取消全选事件
            ...
            // 查询当前页面表格指定列值，无需选中该列
            selectColumsNoSelect: function(column){
            	var rows = $.map($.btTable.bootstrapTable('getData'), function (row) {
        	        return row[column];
        	    });
            	if ($.common.isNotEmpty($.table._option.rememberSelected) && $.table._option.rememberSelected) {
            		rows = rows.concat(selectionIds);
        ...
        operate: {
            ...
            // 无“加载中”等弹窗提示的提交
            submitNoloading: function(url,type,dataType,data,callback){
            	var config = {
        	        url: url,
        	        type: type,
        	        dataType: dataType,
            ...
            // 无“加载中”等弹窗提示的POST提交
            postNoloading: function(url, data, callback) {
            	$.operate.submitNoloading(url, "post", "json", data, callback);
            },
            // 无“加载中”等弹窗提示的GET提交
            getNoloading: function(url, callback) {
            	$.operate.submitNoloading(url, "get", "json", "", callback);
            },
            // 详细信息
            ...
            detailTab: function (id) {
                $.modal.openTab($.table._option.modalName+"详情", $.operate.detailUrl(id));
            },
            // 删除信息
            remove: function(id) {
            ...
            addUrl: function(id) {
				console.log('id:::' + id)
				console.log('url::' + $.table._option.createUrl)
